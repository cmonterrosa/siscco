...
class Vencimiento
  ...
  attr_accessor :credito, :pago_diario, :dias_atraso, :moratorio, :gastos_cobranza, :capital_vencido, :cuota_diaria, :fecha_calculo, :intereses_devengados, :devengo_diario, :interes_vencido
  ...
  def procesar
     ...
     dias_transcurridos = (hoy - proximo_pago(credito).fecha_limite.yday).to_i
      ...
      @capital_vencido = @capital_vencido * numero_clientes
      @interes_vencido = @interes_vencido * numero_clientes
       if dias_transcurridos > 0
          @pago_diario = pago_minimo_informativo(credito) / credito.producto.periodo.dias.to_f
          #moratorio_diario = ((credito.producto.moratorio / 100.0 ) * proximo_pago(credito).capital_minimo.to_f ) / credito.producto.periodo.dias.to_f
